...
class Connection {
  ...
  startSocket() {
    ...
    this._socket.on(
      "cmdStdout",
      (id, text) => this.onCmdStdoutHandler && this.onCmdStdoutHandler(id, text)
    );

    this._socket.on(
      "cmdStderr",
      (id, text) => this.onCmdStderrHandler && this.onCmdStderrHandler(id, text)
    );
   ...
   */ getAdapterInstances(adapter, update) {
    ...
    this._promises["instances" + adapter] =
      this._promises["instances" + adapter] ||
 ...
 */
  ...
  sendTo(instance, command, data) {
    return this._sendCmd("sendTo", instance, command, data);
  }
   ...
   */
  extendObject(id, obj) {
    return this._sendRecCmd("extendObject", id, obj);
  } /*   extendObject(id, obj) {
    if (!this.connected) {
      return Promise.reject(NOT_CONNECTED);
    }
 ...
 */
  getForeignStates(pattern) {
    return this._sendRecCmd("getForeignStates", pattern || "*");
  }
 ...
 */
  getForeignObjects(pattern) {
    return this._sendRecCmd("getForeignObjects", pattern || "*");
  }
  /**
 ...
 */
  getHistory(id, options) {
    return this._sendRecCmd("getHistory", id, options);
  }
  /**
   * Get the history of a given state.
   * @param {string} id
 ...
 */
  changePassword(user, password) {
    return this._sendRecCmd("changePassword", user, password);
  }
